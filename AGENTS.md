...
> แนวทางการจัดการ Repository โค้ด

- **repo_url:** "https://github.com/openclaw/openclaw"
- **file_references:** "In chat replies, file references must be repo-root relative only (example: `extensions/bluebubbles/src/channel.ts:80`); never absolute paths or `~/...`."
- **github_comments_format:** "Use literal multiline strings or `-F - <<'EOF'` (or $'...') for real newlines; NEVER embed '\\n'."
- **github_comment_footgun:** "NEVER use `gh issue/pr comment -b '...'` when body contains backticks or shell chars. Always use single-quoted heredoc (`-F - <<'EOF'`) to avoid command substitution/escaping corruption."
- **github_linking_footgun:** "Do NOT wrap issue/PR refs like `#24643` in backticks when auto-linking is desired. Use plain `#24643` (optionally add full URL)."
- **github_searching_footgun:** "Do not limit yourself to the first 500 issues or PRs when wanting to search all. Unless you're supposed to look at the most recent, keep going until you've reached the last page in the search."
- **security_advisory_analysis:** "Before triage/severity decisions, READ `SECURITY.md` to align with OpenClaw's trust model and design boundaries."
...
> กฎการทำลิงก์สำหรับเอกสาร Mintlify

- **docs_hosting:** "Mintlify (docs.openclaw.ai)."
- **internal_doc_links:** "In `docs/**/*.md`: root-relative, no `.md`/`.mdx` (example: `[Config](/configuration)`)."
- **skill_prerequisite:** "When working with documentation, read the mintlify skill."
- **section_cross_references:** "Use anchors on root-relative paths (example: `[Hooks](/configuration#hooks)`)."
- **doc_headings_anchors:** "AVOID em dashes and apostrophes in headings because they break Mintlify anchor links."
- **link_reply_format:** "When Peter asks for links, reply with full `https://docs.openclaw.ai/...` URLs (not root-relative)."
- **docs_touch_reply:** "When you touch docs, end the reply with the `https://docs.openclaw.ai/...` URLs you referenced."
- **github_readme_urls:** "Keep absolute docs URLs (`https://docs.openclaw.ai/...`) so links work on GitHub."
- **generic_content_rule:** "Docs content MUST be generic: NO personal device names/hostnames/paths; use placeholders like `user@gateway-host` and 'gateway host'."
...
> แนวทางการเขียนและรันเทสต์

- **test_framework:** "Vitest with V8 coverage thresholds (70% lines/branches/functions/statements)."
- **file_naming:** "Match source names with `*.test.ts`; e2e in `*.e2e.test.ts`."
- **pre_push_hook:** "Run `pnpm test` (or `pnpm test:coverage`) before pushing when you touch logic."
- **worker_limit:** "Do NOT set test workers above 16; tried already."
- **memory_pressure_fallback:** "If local Vitest runs cause memory pressure (common on non-Mac-Studio hosts), use `OPENCLAW_TEST_PROFILE=low OPENCLAW_TEST_SERIAL_GATEWAY=1 pnpm test` for land/gate runs."
- **live_tests:** "Real keys: `CLAWDBOT_LIVE_TEST=1 pnpm test:live` (OpenClaw-only) or `LIVE=1 pnpm test:live` (includes provider live tests)."
- **docker_tests:** "`pnpm test:docker:live-models`, `pnpm test:docker:live-gateway`. Onboarding Docker E2E: `pnpm test:docker:onboard`."
- **test_documentation:** "Full kit + what’s covered: `docs/testing.md`."
- **changelog_placement:** "In the active version block, append new entries to the end of the target section (`### Changes` or `### Fixes`); do not insert at the top of a section."
- **changelog_policy:** "User-facing changes only; NO internal/meta notes (version alignment, appcast reminders, release process)."
- **test_changelog_exemption:** "Pure test additions/fixes generally do NOT need a changelog entry unless they alter user-facing behavior or the user asks for one."
- **mobile_testing:** "Before using a simulator, check for connected real devices (iOS + Android) and prefer them when available."
...
- **commit_script:** "Create commits with `scripts/committer '<msg>' <file...>`; avoid manual `git add`/`git commit` so staging stays scoped."
- **commit_message_style:** "Follow concise, action-oriented commit messages (e.g., `CLI: add verbose flag to send`)."
- **commit_grouping:** "Group related changes; avoid bundling unrelated refactors."
- **pr_template:** "Canonical PR submission template: `.github/pull_request_template.md`"
- **issue_templates:** "Canonical issue submission templates: `.github/ISSUE_TEMPLATE/`"
...
> การเผยแพร่แพคเกจ NPM และจัดการ 1Password

- **prerequisite_skill:** "Use the 1password skill; all `op` commands MUST run inside a fresh tmux session."
- **signin_op:** "`eval \"$(op signin --account my.1password.com)\"` (app unlocked + integration on)."
- **read_otp:** "`op read 'op://Private/Npmjs/one-time password?attribute=otp'`."
- **npm_publish:** "`npm publish --access public --otp=\"<otp>\"` (run from the package dir)."
- **verify_publish:** "Verify without local npmrc side effects: `npm view <pkg> version --userconfig \"$(mktemp)\"`."
